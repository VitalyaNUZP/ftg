...
class DemotivatorMod(loader.Module):
        ...
        async with message.client.conversation(chat) as conv:
            if text:
                ...
                try:
                    ...
                    return await message.edit("<b>Розблокуй @ShittyQuoteBot</b>")
        if response.text:
    ...
    async def mqcmd(self, message):
        """Цитата з повідомлення 2"""
        bw = not utils.get_args(message)
        reply = await message.get_reply_message()
        if not reply or not reply.raw_text:
            return await message.edit("<b>Відповідь на фото!</b>")
        ...
        text = "“" + text + "„"
        bf = requests.get(
            "http://vitalyatroz.ga/files/module/times.ttf"
        ).content
...
async def cmds(message, type_):
    ...
    if not event:
        if message.client._conversations.get(1376531590) is not None:
            return await message.edit("<b>Зачекайте...</b>")
        ...
        if (
            ...
            return await message.edit("<b>Відповідь на фото/відео/стікер</b>")
        if reply.file.size > 4194304:
            return await message.edit("<b>Файл занадто великий (більше 4 МБ)</b>")
        args = utils.get_args_raw(message) or reply.message
        if not args:
            return await message.edit("<b>Немає тексту</b>")
        if len(args) > 500:
            return await message.edit("<b>Текст занадто довгий (більше 500 символів)</b>")
            ...
            try:
                ...
                if not response.media:
                    if response.raw_text.startswith("[400]"):
                        return await message.edit("<b>Прошу зачекати 10 секунд</b>")
                    response = await conv.wait_event(
                        NewMessage(incoming=True, from_users=CHAT)
                    )

            except YouBlockedUserError:
                return await message.edit(f"<b>Розброкую {CHAT}</b>")

            except (TimeoutError, CancelledError):
                return await message.edit("<b>Бот не відповідає</b>")
    ...
    if not text:
        await message.edit("<b>Відповідь на фото з текстом </b>")
        return
    await message.edit("Обробка...")
    bytes_image = await event.download_media(bytes)
    demotivator = await demotion(font_bytes, bytes_image, text, type_)
    await message.edit("Надсилання...")
    if is_reply:
        await message.delete()
...
async def cmdrands(message, type_):
    ...
    if not event:
        ...
        reply = await message.get_reply_message()
        if not reply:
            await message.edit("<b>Відповідь на медіа</b>")
            return
        try:
            media = reply.media
        except Exception:
            return await message.edit("<b>Тільки медіа</b>")
        ...
        async with message.client.conversation(chat) as conv:
            try:
                ...
                await message.reply("<b>Розблокуй @super_rjaka_demotivator_bot</b>")
                return
            await message.edit("<b>Надсилання...</b>")
            await message.delete()
...
async def procces_img(message, way):
    ...
    if not txt:
        txt = "я лошара."
    if not reply:
        await message.edit("Відповідь на фото/стікер.")
        return
