...
async def downloading(event, big=False):
	...
	if not args:
		if not reply:
			await event.edit("<b>Не вказане посилання!</b>")
			return
		message = reply
	else:
		message = event
	...
	for url in urls:
		try:
			await event.edit("<b>Завантаження...</b>\n"+url)
			fname = url.split("/")[-1]
			...
			if big:
				...
				for chunk in text.iter_content(1024):
					f.write(chunk)
				f.close()
				await event.edit("<b>Відправлення...</b>\n"+url)
			...
			await event.reply("<b>Помилка про завантажені!</b>\n"+url+"\n<code>"+str(e)+"</code>")
