...
class FileUploaderMod(loader.Module):
    """Завантаження файлів до Telegram""" 

    strings = {"name": "Файловый завантажувач"}
    ...
    async def x0cmd(self, message):
        ...
        if not reply:
            await message.edit("<b>Реплай на файл</b>")
            return
        media = reply.media
        if not media:
        ...
        try:
            ...
            await message.edit("<b>Помилка</b>")
            return
        url = x0at.text
    ...
    async def telegraphcmd(self, message):
        ...
        if message.is_reply:
            ...
            if isinstance(data, bool):
                await message.edit("<b>Реплай на фото/відео</b>")
                return
        else:
            await message.edit("<b>Реплай на фото/відео</b>")
            return
    ...
    async def imgurcmd(self, message):
        """Завантажувач на imgur"""
        chat = "@ImgUploadBot"
        reply = await message.get_reply_message()
        async with message.client.conversation(chat) as conv:

            if not reply:
                await message.edit("<b>Реплай на фото</b>")
                return
            else:
                pic = await check_mediaa(message, reply)
                if not pic:
                    await utils.answer(message, "<b>Реплай на фото</b>")
                    return
            await message.edit("<b>Завантаження...</b>")
            try:
                what = lol(pic)
                ...
                await message.edit("<code>Розблокуй @imgurbot_bot</code>")
                return
