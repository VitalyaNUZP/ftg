...
class TerminalMod(loader.Module):
    """Запускає команду в терміналі"""

    strings = {
        "name": "Terminal",
        "flood_wait_protect_cfg_doc": "Затримка між повідомленнями, щоб уникнути заблокування",
        "what_to_kill": "<b>Що вбити?</b>",
        "kill_fail": "<b>Не вдалося вбити процес</b>",
        "killed": "<b>Вбито</b>",
        "no_cmd": "<b>Немає команди, що виконується в цьому повідомленні</b>",
        "running": "<b>Команда:</b> <code>{}</code>",
        "finished": "\n<b>Код:</b> <code>{}</code>",
        "stdout": "\n<b>Stdout:</b>\n<code>",
        "stderr": "</code>\n\n<b>Stderr:</b>\n<code>",
        "end": "</code>",
        "auth_fail": "<b>Не вдалося авторизуватися</b>",
        "auth_needed": '<a href="tg://user?id={}">Interactive authentication required</a>',
        "auth_msg": (
            "<b>Введіть код авторизації, який ви отримали в Telegram</b>\n" 
            "<code>{}</code> <b>виконати</b> <code>{}</code>"
        ),
        "auth_locked": "<b>Авторизація заблокована</b>",
        "auth_ongoing": "<b>Авторизація...</b>",
        "done": "<b>Готово</b>",
    }
    ...
    async def aptcmd(self, message):
        """Инсталює пакети через .apt <пакети>"""
    ...
    async def terminatecmd(self, message):
        """Надсилає сигнал SIGTERM процесу, що виконується в цьому повідомленні"""
    ...
    async def killcmd(self, message):
        """Надсилає сигнал SIGKILL процесу, що виконується в цьому повідомленні"""
        if not message.is_reply:
            await utils.answer(message, self.strings("what_to_kill", message))
            return
        if hash_msg(await message.get_reply_message()) in self.activecmds:
    ...
    async def neofetchcmd(self, message):
        """Надсилає відповідь з результатом команди neofetch"""
        await self.run_command(
            message,
            "neofetch --stdout",
            RawMessageEditor(
    ...
    async def uptimecmd(self, message):
        """Показує час, який пройшов з моменту запуску бота"""
        await self.run_command(
            message,
            "uptime",
