...
# requires: moviepy requests | pip install moviepy requests
...
class VideoEditorMod(loader.Module):
    "Модуль для роботи з відео"

    strings = {
        "name": "Редактор відео",
        "downloading": "<b>[{}]</b> Завантаження...",
        "working": "<b>[{}]</b> Працює...",
        "exporting": "<b>[{}]</b> Експорт...",
        "set_value": "<b>[{}]</b> Укажіть рівень від {} до {}...",
        "reply": "<b>[{}]</b> відповісти на відео/gif...",
        "set_time": "<b>[{}]</b> Вкажіть час у форматі початок(ms):кінець(ms)",
        "set_link": "<b>[{}]</b> Введіть посилання...",
    }

    @loader.owner
    async def xflipvcmd(self, m: types.Message):
        """.xflipv <reply_to_video> - Перевернути відео на X"""
    ...
    async def yflipvcmd(self, m: types.Message):
        """.yflipv <reply_to_video> - Перевернути відео на Y"""
    ...
    async def bwvcmd(self, m: types.Message):
        """.bwv <reply_to_video> - Чорний білий"""
    ...
    async def revvcmd(self, m: types.Message):
        """.revv <reply_to_video> - Зворотне відео"""
    ...
    async def paintvcmd(self, m: types.Message):
        """.paintv <reply_to_video> - Ефект фарби"""
    ...
    async def invertvcmd(self, m: types.Message):
        """.invertv <reply_to_video> - Інвертуйте кольори"""
    ...
    async def rmsvcmd(self, m: types.Message):
        """.rmsv <reply_to_video> - Прибрати звук (в gif без стиснення)"""
    ...
    async def cutvcmd(self, m: types.Message):
        """.cutv <int [За замовчуванням 30]> <reply_to_video> - Вирізати відео"""
    ...
    async def audvcmd(self, m: types.Message):
        """.audv <link> <reply_to_video> - Додайте аудіо до відео"""
        args = utils.get_args_raw(m)
        if not args:
    ...
    async def fpsvcmd(self, m: types.Message):
        """.fpsv <int [За замовчуванням 30]> <reply_to_video> - Зміна fps"""
        args = utils.get_args_raw(m)
        if not args:
            fps = 30
    ...
    async def marginvcmd(self, m: types.Message):
        """.marginv <int [За замовчуванням 5]> <reply_to_video> - Додати поле"""
        args = utils.get_args_raw(m)
        if not args:
            margin = 5
    ...
    async def speedvcmd(self, m: types.Message):
        """.speedv <float [За замовчуванням 1.5]> <reply_to_video> - Швидкість"""
        args = utils.get_args_raw(m)
        if not args:
            speed = 1.5
    ...
    async def contrastvcmd(self, m: types.Message):
        """.contrastv <float [За замовчуванням 1.5]> <reply_to_video> - Контраст"""
        args = utils.get_args_raw(m)
        if not args:
            contrast = 1.5
    ...
    async def lumvcmd(self, m: types.Message):
        """.lumv <float [За замовчуванням 25]> <reply_to_video> - Лум"""
        args = utils.get_args_raw(m)
        if not args:
            lum = 25
        elif re.match(r"^\d+(\.\d+)?$", args) and (0.009 < float(args) < 100.1):
    ...
    async def scalevcmd(self, m: types.Message):
        """.scalev <float [За замовчуванням 0.75]> <reply_to_video> - масштаб("Змінити розмір") відео"""
        args = utils.get_args_raw(m)
        if not args:
            scale = 0.75
        elif re.match(r"^\d+(\.\d+)?$", args) and (0.009 < float(args) < 100.1):
